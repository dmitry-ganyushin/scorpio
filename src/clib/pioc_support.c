...
void adios_get_step_info(file_desc_t *file, size_t, size_t, size_t);
...
int adios_read_global_dimensions(iosystem_desc_t *ios, file_desc_t * file, char **var_names, size_t var_size)
{
...
int adios_get_attrs(file_desc_t *file, int attr_id, char *const *attr_names, size_t i)
{
    ...
    if (attr) {
        adios2_type type;
        adios2_error err_attr_type = adios2_attribute_type(&type, attr);
        if (err_attr_type != adios2_error_none) {
            return pio_err(NULL, file, PIO_EADIOS2ERR, __FILE__, __LINE__,
...
int adios_get_dim_ids(file_desc_t *file, size_t var_id)
{/*reading dims */
    {
        ...
        if (attr) {
            ...
            if (size_attr > 0) {
                ...
                if (file->adios_vars[var_id].gdimids == NULL)
                    file->adios_vars[var_id].gdimids = malloc(
                            file->adios_vars[var_id].ndims * sizeof(int));

                for (size_t a = 0; a < size_attr; a++) {
                    int gdimid = get_dim_id(file, attr_data[a]);
                    if (gdimid != -1) {
                        file->adios_vars[var_id].gdimids[a] = gdimid;
                    } else {
...
void adios_get_ndims(file_desc_t *file, size_t var_id)
{/*get ndims*/
    {
...
void adios_get_adios_type(file_desc_t *file, size_t var_id)
{/****** get adios type *****/
    {
        char *attr_name = adios_name(adios_pio_var_prefix, file->adios_vars[var_id].name,
...
void adios_get_nc_type(file_desc_t *file, size_t var_id)
{/***** get nc_type ******/
    {
        char *attr_name = adios_name(adios_pio_var_prefix, file->adios_vars[var_id].name,
...
void update_interval_map(adios_interval_map_t *map, short min_frame_id, short max_frame_id, short adios_step)
{
    map->map[adios_step][0] = min_frame_id;
    map->map[adios_step][1] = max_frame_id;
}
adios_interval_map_t* init_interval_map(size_t n_steps)
{
    adios_interval_map_t* map = (adios_interval_map_t*) malloc(sizeof (adios_interval_map_t));
    map->n_adios_steps = n_steps;
    map->map = (short **)(malloc(n_steps * sizeof (short*)));
    for (size_t i = 0; i < map->n_adios_steps; i++){
        map->map[i] = (short *)malloc( 2 * sizeof (short));
        map->map[i][0] = -1;
        map->map[i][1] = -1;
    }
    return map;
}
void adios_get_step_info(file_desc_t *file, size_t var_id, size_t adios_step, size_t n_adios_steps) {       /***** get relevant adios step ******/
    assert(file->adios_vars[var_id].interval_map == NULL);
    file->adios_vars[var_id].interval_map = init_interval_map(n_adios_steps);
                        ...
                        int mode, int retry)
{
 ...
 if ( *iotype == PIO_IOTYPE_ADIOS) {
    ...
    if (filename[0] != '/') /* Restart files are in the current run directory, so their paths do not start with slash */
        file->iotype = PIO_IOTYPE_ADIOS; /* Hack: Always read restart files with ADIOS type */
    if (file->iotype == PIO_IOTYPE_ADIOS)
    {
        ...
        if (0 == stat(bpname, &sd)) {
            ...
            file->ioH = adios2_declare_io(ios->adios_readerH, file->io_name);
            if (file->ioH == NULL) {
                return pio_err(ios, NULL, PIO_EADIOS2ERR, __FILE__, __LINE__,
                               "Declaring (ADIOS) IO (name=%s) failed for file (%s)",
                               declare_name, pio_get_fname_from_file(file));
    ...
    *ncidp = pio_add_to_file_list(file, comm);
    ...
    if(file->iotype == PIO_IOTYPE_ADIOS) {
        ...
        while ( step < nsteps && adios2_begin_step(file->engineH, adios2_step_mode_read, -1.,
